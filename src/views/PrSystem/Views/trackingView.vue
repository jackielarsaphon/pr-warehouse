...
async function applyReconcile(plan) {
  ...
  // รวม patch ตาม id (เผื่อแถวเดียวกันถูกทั้ง refresh + รับงานมือ)
  const byId = new Map()
  ...
  for (let i = 0; i < updates.length; i += 10) {
    await Promise.all(updates.slice(i, i + 10).map((u) => supabase.from('ap_requests').update(u.patch).eq('id', u.id)))
  }

  const cutIds = plan.cut.map((r) => r.id)
  if (cutIds.length) {
    const { error } = await supabase.from('ap_requests').delete().in('id', cutIds)
    if (error) throw error
  }

  const orphanIds = plan.orphan.map((r) => r.id)
  if (orphanIds.length) {
    const { error } = await supabase.from('ap_requests').update({ is_orphaned: true, orphaned_at: nowIso }).in('id', orphanIds)
    if (error) throw error
  }
}
...
async function syncWithTrcloud() {
  ...
  syncing.value = true
  try {
    if (!trcloudStore.apItemRows.length) {
    ...
    const plan = reconcile(trcloudStore.apItemRows, rows.value, { includeStatus: true })
    const { stats } = plan
