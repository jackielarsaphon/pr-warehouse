      ...
      <div class="overflow-x-auto">
        <table class="w-full text-[13px] min-w-[1540px] border-collapse">
          ...
          <tbody>
            ...
            <tr v-if="loading">
              <td :colspan="selectionMode ? 16 : 15" class="px-4 py-10 text-center" style="color: var(--color-text-muted)">กำลังโหลดข้อมูล...</td>
            </tr>
            <tr v-else-if="!loading && totalRows === 0">
              <td :colspan="selectionMode ? 16 : 15" class="px-4 py-10 text-center" style="color: var(--color-text-muted)">ไม่พบข้อมูล</td>
            </tr>
          </tbody>
        </table>
      </div>
